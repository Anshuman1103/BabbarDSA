...
void dfs(vector<vector<int>> &graph, int startNode, vector<int> &ans,
         vector<int> &visited) {
  visited[startNode] = 1;
...
vector<int> graphDFS(vector<vector<int>> &graph, int startNode) {
  vector<int> ans;
  vector<int> visited(graph.size() + 1, 0);
  dfs(graph, startNode, ans, visited);
  return ans;
}
...
int main() {
  ...
  graphBFS(graph, 3);
  vector<int> ans = graphDFS(graph, 3);
  cout << "\nPrinting DFS" << endl;
  for (auto node : ans) {
    cout << node << " ";
  }
